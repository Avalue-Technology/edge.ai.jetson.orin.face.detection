...
class QuickReceiver():
    def __init__(self):
        ### AI ###
        strModelName = './model/FACE_LANDMARK_5KPS/QDEEP.OD.FACE.LANDMARK.5KPS.CFG'
        QDEEP.QDEEP_CREATE_OBJECT_DETECT(QDeepGPUType.NVIDIA, 0 ,QDeepObjectDetectConfigModel.QDEEP_OBJECT_DETECT_CONFIG_MODEL_FACE_LANDMARK_5_KEYPOINTS, 
                                         strModelName.encode('utf-8'),byref(self.m_detector), QDeepObjectDetectFlag.FULL | QDeepObjectDetectFlag.BEHAVIOR)
        ...
        if video_type == 0:
            video_path = "./model/FACE_LANDMARK_5KPS/people-counting.mp4"
             # 確保檔案存在
        ...
        while True:
            ...
            QDEEP.QDEEP_SET_VIDEO_OBJECT_DETECT_UNCOMPRESSION_BUFFER(self.m_detector, QDEEP_COLORSPACE_TYPE_BGR24, c_ulong(self.m_nWidth), c_ulong(self.m_nHeight), pImageBuffer, c_ulong(bufferlen), 
                                                                     self.m_pObjectList, byref(self.nObjectSize), 1)
            # print("self.nObjectSize is ", self.nObjectSize)

            for i in range(self.nObjectSize.value):
                    object_color = (0, 0, 255)
                    cv2.rectangle(frame, (self.m_pObjectList[i].nX, self.m_pObjectList[i].nY), (self.m_pObjectList[i].nX+self.m_pObjectList[i].nWidth, self.m_pObjectList[i].nY+self.m_pObjectList[i].nHeight), 
